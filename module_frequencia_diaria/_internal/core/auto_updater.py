...
def check_and_download_update():
    ...
    # CRITICAL FIX: Ler versão do app_root (pasta onde esta o .exe instalado),
    # NÃO do base_dir (sys._MEIPASS que é temporário e read-only dentro do bundle PyInstaller).
    # Isso evita o loop infinito de updates onde a versao embutida no bundle nunca muda.
    ...
    if not os.path.exists(version_path):
        fallback_path = os.path.join(app_root, 'core', 'version.json')
        if os.path.exists(fallback_path):
            version_path = fallback_path
    # Ultimo fallback: tenta no _MEIPASS (apenas leitura inicial)
    if not os.path.exists(version_path):
