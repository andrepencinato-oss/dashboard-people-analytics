...
def get_drive_service():
    ...
    if not os.path.exists(creds_path):
        if getattr(sys, 'frozen', False):
            ...
            creds_path = os.path.join(app_root, 'core', 'token.json')
            if not os.path.exists(creds_path):
...
def check_and_download_update():
    ...
    ota_config_candidates = [
        os.path.join(app_root, 'core', 'ota_config.json'),
        os.path.join(base_dir, 'core', 'ota_config.json')
    ]
    ota_config_path = next((p for p in ota_config_candidates if os.path.exists(p)), None)

    version_candidates = [
        os.path.join(app_root, 'core', f'version_{app_name}.json'),
        os.path.join(app_root, 'version.json'),
        os.path.join(base_dir, 'core', f'version_{app_name}.json'),
        os.path.join(base_dir, 'core', 'version.json')
    ]
    version_path = next((p for p in version_candidates if os.path.exists(p)), None)
    
    if not ota_config_path:
        return False
