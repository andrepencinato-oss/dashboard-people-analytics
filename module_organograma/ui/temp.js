      ...
      dirData.ccs.forEach(cc => {
          ...
          Object.keys(roleGroups).sort().forEach(cargo => {
              ...
              colabs.forEach(e => {
                  ...
                  colabsHtml += `<div class="colab-item"><div class="colab-info"><div class="colab-name">${e.nome}${badgeInfo}</div><div class="colab-tenure">${tenure}</div></div><div class="colab-salary">${fmtMoney(e.salario)}</div></div>`;
              });
              
              const safeId = (cc.ccCod + '_' + cargo).replace(/[^a-zA-Z0-9]/g, '');
const hiddenDataHtml = `<div id="data-${safeId}" style="display:none;">${colabsHtml}</div>`;
...
function switchView(view) {
  ...
  if (headerSubtitle) {
      headerSubtitle.innerText = view === 'matrix' ? 'Árvore Matricial' : 'Visão por Centro de Custo';
  }
...
function buildMatrixTree(ccsToRender) {
  ...
  const renderNode = (node) => {
    ...
    if(node.isCC) {
      ...
      html += `
        <div class="matrix-card" style="border-top:3px solid var(--blue); min-width: 140px; max-width: 180px;">
  ...
  populateMenu('menuFilterDir', Object.keys(allDirs), allDirs, 'Todas as Diretorias', 'Dir');
  ...
  populateMenu('menuFilterGer', Object.keys(dictGers), dictGers, 'Todas as Gerências', 'Ger');
  ...
  populateMenu('menuFilterCoo', Object.keys(dictCoos), dictCoos, 'Todas as Coordenações', 'Coo');
  ...
  populateMenu('menuFilterLid', Object.keys(dictLids), dictLids, 'Todas as Lideranças', 'Lid');
  ...
  if (targetNode === rootNode) {
      ...
      if(dirKeys.length === 1) {
          finalHtml += renderNode(allDirs[dirKeys[0]], pathNodes, targetNode, false);
      } else {
...
function populateMenu(menuId, keys, dict, defaultText, levelStr) {
  const ul = document.getElementById(menuId);
  if(!ul) return;
  const currentVal = document.getElementById('matrixFilter' + levelStr).value;
  
  let html = `<li class="${currentVal === 'all' || !currentVal ? 'selected' : ''}" onclick="handleMenuSelect('${levelStr}', 'all')">${defaultText}</li>`;
  
  const uniqueNames = new Set();
  keys.forEach(k => {
      const child = dict[k];
      if(child && child.name && !uniqueNames.has(child.name)) {
          uniqueNames.add(child.name);
          const isSelected = currentVal === child.name;
          const escapedName = child.name.replace(/'/g, "\\'");
          html += `<li class="${isSelected ? 'selected' : ''}" onclick="handleMenuSelect('${levelStr}', '${escapedName}')" title="${child.name}">${child.name}</li>`;
      }
  });
  ul.innerHTML = html;
}
